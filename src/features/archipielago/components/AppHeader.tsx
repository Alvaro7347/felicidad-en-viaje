        ...
        >
          {items.map((it) => (
            <button
              ...
              onMouseEnter={(e) => {
                if (!it.disabled) e.currentTarget.style.background = "#F5FBF8";
              }}
              onMouseLeave={(e) => {
...
export function AppHeader({
  ...
  const headerTripTitle =
    isAccompanied && trimmedStudent ? `Viaje de ${trimmedStudent}` : "Tu viaje musical";
  ...
  const modernHeaderScreens: Screen[] = [
    ...
    "mission-nine",
    "celebration",
    "first-melodies-island",
    "first-melodies-lesson",
    "pulse-island",
    "pulse-lesson",
    "rhythm-island",
    "rhythm-lesson",
          ...
          >
            ...
            {!isMenuScreen && islandId && (
              <>
                {" "}
                ·{" "}
                <span style={{ color: B.greenDark, fontWeight: 700 }}>
                  {pct}% {completionText}
                </span>
              </>
            )}
