...
import {
  ...
  FIRST_MELODIES_SUMMARY_CHIPS,
  CLOSURE_QUESTIONS,
  type Lesson,
} from "../data/firstMelodiesLessons";

// ─────────────────────────────────────────────────────────────
// Bloque: Video (reutilizable dentro de la lección)
// ─────────────────────────────────────────────────────────────
function VideoBlock({ lesson }: { lesson: Lesson }) {
  const [open, setOpen] = useState(false);

  if (lesson.videoPending || !lesson.videoId) {
    return (
      <Card style={{ background: B.pinkLight, border: `1px dashed ${B.pink}` }}>
        ...
        <p style={{ margin: 0, fontSize: 13.5, lineHeight: 1.6, color: B.dark }}>
          Este video será agregado pronto. Aquí aprenderás {lesson.description.toLowerCase()}
        </p>
      </Card>
    );
  }
...
// ─────────────────────────────────────────────────────────────
// Bloque: Cierre / resumen / medición / premio
// ─────────────────────────────────────────────────────────────
function ClosureBlock() {
    ...
    if (typeof window !== 'undefined') {
      try {
        window.localStorage.setItem(
          'archipielago_first_melodies_closure',
          JSON.stringify({ feeling, hardest, readiness, at: Date.now() }),
        );
      } catch {
        // localStorage no disponible: se mantiene solo el estado local.
      }
    }
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <Card>
        <div style={{ fontSize: 11, fontWeight: 800, color: B.greenDark, letterSpacing: '1.2px', textTransform: 'uppercase', marginBottom: 10 }}>
          Lo que ya conquistaste
        </div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
          {FIRST_MELODIES_SUMMARY_CHIPS.map((chip) => (
            <span
              key={chip}
              style={{
      ...
      <QuestionBlock
        question={CLOSURE_QUESTIONS.feeling.question}
        options={CLOSURE_QUESTIONS.feeling.options}
        value={feeling}
        onChange={setFeeling}
        disabled={done}
      />
      <QuestionBlock
        question={CLOSURE_QUESTIONS.hardest.question}
        options={CLOSURE_QUESTIONS.hardest.options}
        value={hardest}
        onChange={setHardest}
        disabled={done}
      />
      ...
      <Card>
        <div style={{ fontWeight: 800, fontSize: 13.5, color: B.dark, marginBottom: 10 }}>
          {CLOSURE_QUESTIONS.readiness.question}
        </div>
        <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
          {CLOSURE_QUESTIONS.readiness.labels.map((label, i) => {
            const value = i + 1;
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
            ...
            <div>
              ...
              <div style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 800, fontSize: 18, color: B.dark }}>
                Insignia: Primeras Melodías
              </div>
            </div>
          </div>
          <p style={{ margin: '0 0 8px', fontSize: 13.5, lineHeight: 1.6, color: B.dark }}>
            Ya no solo conoces tu ukelele: comenzaste a hacerlo sonar con tus primeros acordes y canciones.
          </p>
          <div style={{ fontSize: 12, color: B.greenDark, fontWeight: 700 }}>
            Microvictoria desbloqueada: tus primeras melodías ya comenzaron.
          </div>
        </Card>
      )}
    </div>
...
// Screen renderer
// ─────────────────────────────────────────────────────────────
export function FirstMelodiesLessonScreen({
  lessonId,
  onBackToIsland,
}: {
  lessonId: string;
  ...
  if (!lesson) {
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        <BackBtn label="Isla de Primeras Melodías" onClick={onBackToIsland} />
        <Card>
  ...
  const total = FIRST_MELODIES_LESSONS.length;
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      {lesson.kind === 'community' && <CommunityBlock />}
      {lesson.kind === 'closure' && <ClosureBlock />}
